...
const SignUpPage1 = () => {
  ...
  const handleSignup = () => {
    ...
    if (!fname && !userErr.fname)
      return setUserErr({ ...userErr, fname: true });
    if (!lname && !userErr.lname)
      return setUserErr({ ...userErr, lname: true });
    if (!email && !userErr.email)
      return setUserErr({ ...userErr, email: true });
    if (!password && !userErr.password)
      return setUserErr({ ...userErr, password: true });
